...
<div class="row container">
  ...
  <div class="img_box col s12 m3">
    <form action='{{url("admin/img/$image->id")}}' method="post" enctype="multipart/form-data">
      ...
      @method('DELETE')
      <div class="col s12">
        <img class="responsive-img" src='{{ asset("img/$path[7]") }}'>
      </div>

      <div class="col s12">
        <label>
          <input data-id="{{ $image->id }}" class="with-gap" name="publish" type="radio" value="1" {{ $image->publish == 1 ? "checked" : "" }}>
          <span>發佈</span>
        </label>
        <label>
          <input data-id="{{ $image->id }}" class="with-gap" name="publish" type="radio" value="0" {{ $image->publish == 0 ? "checked" : "" }}>
          <span>不發佈</span>
        </label>
      </div>
